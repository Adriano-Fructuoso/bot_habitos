...
class DailyRating(Base):
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint("user_id", "date", name="uq_daily_rating_user_date"),
    )
...
class ProcessedCallback(Base):
    ...
    id = Column(Integer, primary_key=True)
    callback_id = Column(String(128), nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)

    __table_args__ = (UniqueConstraint("callback_id", name="uq_callback_id"),)
...
class Reminder(Base):
    ...
    days = Column(String(20), nullable=False)  # "1,2,3,4,5" (segunda=1, domingo=7)
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint("user_id", "habit_id", name="uq_reminder_user_habit"),
    )
